...
int main(int argc, char** argv) {
    ...
    const auto model_dir = std::filesystem::current_path() / "modelfiles" / "qwen3_vl_8b_instruct";

    geniex::QnnRuntimeConfig runtime_cfg;
    geniex::VLMConfig        config;

    config.llm_config.model_paths = {
        (model_dir / "part1_of_6.bin").string(),
        (model_dir / "part2_of_6.bin").string(),
        (model_dir / "part3_of_6.bin").string(),
        (model_dir / "part4_of_6.bin").string(),
        (model_dir / "part5_of_6.bin").string(),
        (model_dir / "part6_of_6.bin").string(),
    };
    ...
    std::cout << "\033[1;36mLoading Qwen3-VL-8B-Instruct...\033[0m\n";
