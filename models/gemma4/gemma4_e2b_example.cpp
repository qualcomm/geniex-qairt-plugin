...
static void printUsage(const char* prog) {
    std::cout << "Usage: " << prog << " [OPTIONS]\n"
              << "  --model-dir <path> QAIRT bundle directory (default: ./modelfiles/gemma4_e2b)\n"
...
static bool parseArgs(int argc, char** argv, Args& args) {
    ...
    if (args.model_dir.empty()) args.model_dir = fs::current_path() / "modelfiles" / "gemma4_e2b";
    return true;
}
...
static void runTurn(geniex::LLMPipeline& pipe, const std::string& user_text, const Args& args) {
    ...
    if (args.chat) {
        try {
            prompt = pipe.applyChatTemplate({{geniex::Role::User, user_text}}, {});
...
int main(int argc, char** argv) {
    ...
    std::cout << "\033[1;36mLoading Gemma4-E2B from " << args.model_dir.string() << "...\033[0m\n";
