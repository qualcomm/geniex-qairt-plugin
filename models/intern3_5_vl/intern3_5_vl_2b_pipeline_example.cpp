...
static void printUsage(const char* prog) {
    std::cout << "Usage: " << prog << " [OPTIONS]\n"
              << "  --model-dir <path>    Bundle directory (default modelfiles/intern3_5_vl_2b)\n"
...
int main(int argc, char** argv) {
    ...
    const auto model_dir = args.model_dir.empty() ? std::filesystem::current_path() / "modelfiles" / "intern3_5_vl_2b"
    ...
    // Shard order and tokenizer/HTP paths come from the bundle's
    // genie_config.json.
    ...
    std::cout << "\033[1;36mLoading Intern3.5-VL-2B...\033[0m\n";
    auto maybe_pipe = geniex::intern3_5_vl::makePipeline(runtime_cfg, config);
    if (!maybe_pipe) {
    ...
    while (true) {
        ...
        // Close the *previous* assistant turn before this turn's delta.
        //
        // The KV cache is retained across turns, so each turn only prefills the
        // newly rendered text. Generation stops on an end-of-generation token
        // before that token reaches the KV cache, so the `<|im_end|>` that
        // terminated the last assistant turn is missing from the cached prefix.
        // Re-emitting it here keeps the ChatML framing well-formed.
        if (!first_turn) formatted = "<|im_end|>\n" + formatted;
        first_turn = false;
