...
int main(int argc, char** argv) {
    ...
    XTUtils arn_utils_0(SEQ_LEN_ARN, KV_LEN_ARN, HIDDEN_SIZE, NUM_HEADS,
                        NUM_KV_HEADS, HEAD_DIM, 0, 17, VOCAB_SIZE,
                        "input_embeds", "_Add_88_Add_output_0", ROPE_THETA, EOS_TOKEN);
    XTUtils arn_utils_1(SEQ_LEN_ARN, KV_LEN_ARN, HIDDEN_SIZE, NUM_HEADS,
                        NUM_KV_HEADS, HEAD_DIM, 18, 35, VOCAB_SIZE,
                        "_Add_88_Add_output_0", "logits", ROPE_THETA, EOS_TOKEN);
    XTUtils ar1_utils_0(SEQ_LEN_AR1, KV_LEN_AR1, HIDDEN_SIZE, NUM_HEADS,
                        NUM_KV_HEADS, HEAD_DIM, 0, 17, VOCAB_SIZE,
                        "input_embeds", "_Add_88_Add_output_0", ROPE_THETA, EOS_TOKEN);
    XTUtils ar1_utils_1(SEQ_LEN_AR1, KV_LEN_AR1, HIDDEN_SIZE, NUM_HEADS,
                        NUM_KV_HEADS, HEAD_DIM, 18, 35, VOCAB_SIZE,
                        "_Add_88_Add_output_0", "logits", ROPE_THETA, EOS_TOKEN);
    ...
    while (true) {
...

        {
            ...
            for (const auto& name : arn_utils_0.input_tensor_names)
                arn_0_input_data.push_back(d0[name]);

            auto d1 = arn_utils_1.prepare_inputs(dummy_arn_embeds, 0, SEQ_LEN_ARN);
            for (const auto& name : arn_utils_1.input_tensor_names)
                arn_1_input_data.push_back(d1[name]);
            ...
            for (const auto& name : ar1_utils_0.input_tensor_names)
                ar1_0_input_data.push_back(d2[name]);

            auto d3 = ar1_utils_1.prepare_inputs(dummy_ar1_embeds, 0, 1);
            for (const auto& name : ar1_utils_1.input_tensor_names)
                ar1_1_input_data.push_back(d3[name]);
        }
        ...
        try {
            while (tokens_processed < curr_len) {
                std::size_t tokens_in_chunk =
                    std::min(SEQ_LEN_ARN, curr_len - tokens_processed);
                last_chunk_size = tokens_in_chunk;

                std::vector<int32_t> chunk_tokens(
                    input_ids.begin() + tokens_processed,
                    input_ids.begin() + tokens_processed + tokens_in_chunk);
                ...
                arn_utils_0.update_kv_cache(arn_0_input_data, arn_0_output_data,
                    static_cast<int>(n_past - tokens_in_chunk),
                    static_cast<int>(tokens_in_chunk), SEQ_LEN_ARN);
                arn_utils_1.update_kv_cache(arn_1_input_data, arn_1_output_data,
                    static_cast<int>(n_past - tokens_in_chunk),
                    static_cast<int>(tokens_in_chunk), SEQ_LEN_ARN);
            ...
            auto logits_view = xt::view(
                xt::adapt(
                    arn_1_output_data[arn_utils_1.output_tensor_order[arn_utils_1.out_states_name_]],
                    std::vector<std::size_t>{1, SEQ_LEN_ARN, VOCAB_SIZE}),
                0, last_chunk_size - 1, xt::all());
            ...
            ar1_utils_0.transfer_kv_cache(ar1_0_input_data, arn_0_input_data,
                                          static_cast<int>(n_past));
            ar1_utils_1.transfer_kv_cache(ar1_1_input_data, arn_1_input_data,
                                          static_cast<int>(n_past));

            // ── Decode loop (AR1, seq_len=1) ─────────────────────────────────
            for (int32_t i = 0;
                 i < args.max_tokens && next_token != EOS_TOKEN && next_token != 151643;
                 ++i)
            {
                ...
                // Update KV caches.
                ar1_utils_0.update_kv_cache(ar1_0_input_data, ar1_0_output_data,
                    static_cast<int>(n_past - 1), 1, 1);
                ar1_utils_1.update_kv_cache(ar1_1_input_data, ar1_1_output_data,
                    static_cast<int>(n_past - 1), 1, 1);

                // Sample next token.
                auto logits_ar1 = xt::view(
                    xt::adapt(
                        ar1_1_output_data[ar1_utils_1.output_tensor_order[ar1_utils_1.out_states_name_]],
        ...
        if (got_first_token) {
            ...
            const size_t decode_tok = generated_tokens.size() > 1
                                        ? generated_tokens.size() - 1 : 0;
            const double tps        = decode_ms > 0.0
                                        ? decode_tok / (decode_ms / 1000.0) : 0.0;
