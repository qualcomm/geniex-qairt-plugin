...
class reg_op_node {
  ...
  public:
    ...
    constexpr reg_op_node(op_reg_parms const p, uint16_t const n, uint16_t const t, uint16_t const l, bool s, bool g,
                          std::string_view const reg) noexcept
        : op_parms(p), op_name_offset(n), type_tag_offset(t), line(l), is_external(s), is_legacy(g), target_reg(reg)
    {
    }

    constexpr reg_op_node(op_reg_parms const p, uint16_t const n, uint16_t const t, uint16_t const l, bool s,
                          bool g) noexcept
