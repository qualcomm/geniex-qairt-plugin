...
// a 'throw'.
NOINLINE inline void throw_deser_error(char const *msg)
{
...
/* the ref deserialize_op_func_ptr is used by the native interface 
   and a pointer to a template-generated kernel specific deserialization function */
using deserialize_op_func_ptr = void *(*)(void *, Deserz &); // Allocation function
/* the deserialize_op_func is used for the Plugin Function Interface 
   which holds a lamda that wrapers the HEXNN_Kernel reference */
using deserialize_op_func = std::function<void *(void *, Deserz &)>;
...
class Deserz : public DeSerError {
  ...
  public:
    ...
    void reset_checkpoint(unsigned value) { op_extra_info.chkpts.second = value; };
