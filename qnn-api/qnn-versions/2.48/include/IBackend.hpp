...
#include "IOTensor/IOTensorTypes.hpp"
...
enum class PerfProfile {
 ...
 BURST                            = 5,
  LLM_DECODE_BURST                      = 6,
  LLM_DECODE_LOW_BALANCED               = 7,
  LLM_DECODE_BALANCED                   = 8,
  LLM_DECODE_DEFAULT                    = 9,
  LLM_DECODE_HIGH_PERFORMANCE           = 10,
  LLM_DECODE_SUSTAINED_HIGH_PERFORMANCE = 11,
  LLM_DECODE_EXTREME_POWER_SAVER        = 12,
  LLM_DECODE_LOW_POWER_SAVER            = 13,
  LLM_DECODE_POWER_SAVER                = 14,
  LLM_DECODE_HIGH_POWER_SAVER           = 15,
...
class IBackend {
 public:
  ...
  virtual bool storeBinaryPathForContext(Qnn_ContextHandle_t contextHandle,
  ...
  virtual bool supportsCustomGraphExecution(Qnn_DeviceHandle_t /*deviceHandle*/) { return false; }

  virtual bool customGraphExecute(
      ...
      const std::vector<qnn::tools::iotensor::GraphInputInfo>& /*graphInputInfos*/) {
    return false;
  }
};
