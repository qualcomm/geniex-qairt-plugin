...
struct API_EXPORT Float16 {
    ...
    constexpr Float16(const Float16 &f) noexcept : d(f.d) {}
    constexpr Float16 &operator=(Float16 f) noexcept;
    ...
    constexpr int16_t frac() const noexcept;
    constexpr uint16_t raw() const noexcept { return d; }

    static constexpr int exp_max() noexcept { return 15; }
    static constexpr int exp_min() noexcept { return -14; }
    static constexpr int16_t bias() noexcept { return 15; }

    static constexpr Float16 zero(bool neg = false) noexcept;
    static constexpr Float16 qnan() noexcept;
    static constexpr Float16 snan() noexcept;
    static constexpr Float16 inf(bool neg = false) noexcept;
    ...
    // instead of inf/nan
    float to_float_alt() const noexcept;
    // same as from-float, but allows +/- 131008 range, using
    // exp=31 as normal.
    static Float16 from_float_alt(float v) noexcept;

  private:
    explicit constexpr Float16(int sign, int exp, int frac) noexcept;

    constexpr uint16_t sign_bit() const noexcept;
    constexpr uint16_t exp_bits() const noexcept;
    constexpr uint16_t frac_bits() const noexcept;

    static constexpr uint16_t make_exp_bits(uint16_t e) noexcept;
    static constexpr uint16_t make_sign_bit(uint16_t s) noexcept;
    static constexpr uint16_t make_frac_bits(uint16_t f) noexcept;

    static constexpr uint16_t make_zero(bool neg) noexcept;
    static constexpr uint16_t make_nan(bool quiet) noexcept;
    static constexpr uint16_t make_inf(bool neg) noexcept;
    static constexpr uint16_t make(int sign, int exp, int frac) noexcept;

    static constexpr uint32_t round(uint32_t v, unsigned s) noexcept;

    constexpr std::pair<int32_t, int32_t> force_norm() const noexcept;
    ...
    friend API_FUNC_EXPORT Float16 operator-(Float16 a) noexcept;
    friend API_FUNC_EXPORT Float16 operator+(Float16 a, Float16 b) noexcept;
    friend API_FUNC_EXPORT Float16 operator-(Float16 a, Float16 b) noexcept;
    friend API_FUNC_EXPORT Float16 operator*(Float16 a, Float16 b) noexcept;
};
...
inline constexpr Float16::Float16(float f) noexcept : d(0)
{
    union U {
...
inline constexpr Float16 &Float16::operator=(const Float16 f) noexcept
{
    d = f.d;
    return *this;
}
...
inline constexpr bool Float16::is_neg() const noexcept
{
    return sign_bit();
}
...
inline constexpr bool Float16::is_subnorm() const noexcept
{
    return exp_bits() == make_exp_bits(0x0000) && frac_bits() != 0x0000;
}

inline constexpr bool Float16::is_norm() const noexcept
{
...
inline constexpr int16_t Float16::exp() const noexcept
{
    assert(is_finite());
...
inline constexpr int16_t Float16::frac() const noexcept
{
    assert(is_finite());
...
inline constexpr Float16 Float16::zero(bool neg) noexcept
{
...
inline constexpr Float16 Float16::inf(bool neg) noexcept
{
...
inline constexpr Float16::Float16(int sign, int exp, int frac) noexcept : d(make(sign, exp, frac)) {}

inline constexpr uint16_t Float16::sign_bit() const noexcept
{
    return d & 0x8000u;
}

inline constexpr uint16_t Float16::exp_bits() const noexcept
{
    return d & 0x7C00u;
}

inline constexpr uint16_t Float16::frac_bits() const noexcept
{
    return d & 0x03FFu;
}

inline constexpr uint16_t Float16::make_sign_bit(uint16_t s) noexcept
{
    return static_cast<uint16_t>(!!s) << 15u;
}

inline constexpr uint16_t Float16::make_exp_bits(uint16_t e) noexcept
{
    return (e & 0x001Fu) << 10u;
}

inline constexpr uint16_t Float16::make_frac_bits(uint16_t f) noexcept
{
    return f & 0x03FFu;
}

inline constexpr uint16_t Float16::make_zero(bool neg) noexcept
{
    return make_sign_bit(neg) | make_exp_bits(0) | make_frac_bits(0);
}

inline constexpr uint16_t Float16::make_nan(bool quiet) noexcept
{
    uint16_t const f = quiet ? 0x0200 : 0x0100;
    return make_sign_bit(0) | make_exp_bits(0x001F) | make_frac_bits(f);
}

inline constexpr uint16_t Float16::make_inf(bool neg) noexcept
{
    return make_sign_bit(neg) | make_exp_bits(0x001F) | make_frac_bits(0x0000);
}

inline constexpr uint16_t Float16::make(int sign, int exp, int frac) noexcept
{
...
inline constexpr uint32_t Float16::round(uint32_t v, unsigned s) noexcept
{
    if (s == 0) return v;
...
inline constexpr std::pair<int32_t, int32_t> Float16::force_norm() const noexcept
{
