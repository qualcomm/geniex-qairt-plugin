...
class IOTensor {
 public:
  ...
  bool initialize(Qnn_ContextHandle_t contextHandle = nullptr, uint32_t dataAlignmentSize = 0);
  ...
  bool tearDownTensors(const qnn_wrapper_api::GraphInfo_t* graph_info) {
    bool status = true;
  ...
  // Manually free current fused buffers (useful before reinitialize to avoid memory leak)
