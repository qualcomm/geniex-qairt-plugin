...
RpcMem::RpcMem(Qnn_ContextHandle_t contextHandle,
...
bool RpcMem::allocateTensorBuffer(Qnn_Tensor_t* tensor, size_t tensorDataSize) {
  ...
  size_t alignedTensorSize = align(tensorDataSize, m_dataAlignmentSize);
  QNN_DEBUG("%s: m_dataAlignmentSize=%u, alignedTensorSize=%u, tensorDataSize=%u",
            __func__,
            m_dataAlignmentSize,
            alignedTensorSize,
            tensorDataSize);
  auto memPointer = m_rpcMemAlloc(RPCMEM_HEAP_ID_SYSTEM, RPCMEM_DEFAULT_FLAGS, alignedTensorSize);
...
bool RpcMem::freeTensorBuffer(Qnn_Tensor_t* tensor) {
  ...
  if (m_sameMemoryFreeTensors.find(tensor) != m_sameMemoryFreeTensors.end()) {
    ...
    // Note: Skip memDeRegister during cleanup as context free already handled it
    // Only clear our internal tracking
...
bool RpcMem::useSameMemory(Qnn_Tensor_t* dest, Qnn_Tensor_t* src) {
  if (nullptr == dest || nullptr == src) {
    QNN_ERROR("RpcMem::useSameMemory Received nullptr");
    return false;
  }

  Qnn_MemHandle_t src_mem_handle = QNN_TENSOR_GET_MEM_HANDLE(src);
  Qnn_MemHandle_t dst_mem_handle = QNN_TENSOR_GET_MEM_HANDLE(dest);

  if (src_mem_handle == nullptr) {
    QNN_ERROR("RpcMem::useSameMemory src tensor has no memory handle registered");
    return false;
  }

  // If destination has a memory handle, attempt to free it
  if (dst_mem_handle != nullptr) {
    if (QNN_SUCCESS != m_qnnInterface->memDeRegister(&dst_mem_handle, 1)) {
      QNN_ERROR("Failed to deregister ion memory with the backend");
      return false;
    }
  }

  // Copy the memory handle and memory type into the destination tensor
  QNN_TENSOR_SET_MEM_TYPE(dest, QNN_TENSOR_GET_MEM_TYPE(src));
  QNN_TENSOR_SET_MEM_HANDLE(dest, src_mem_handle);
...
void* RpcMem::allocateTensorFusedBuffer(uint64_t bufferSize, int32_t* fd) {
  *fd = -1;
  ...
  QNN_DEBUG("%s: m_dataAlignmentSize=%u, alignedBufferSize=%u, original bufferSize=%u",
            __func__,
            m_dataAlignmentSize,
            alignedBufferSize,
            bufferSize);
...
void RpcMem::freeFusedBuffers() {
  ...
  // Note: Memory handles are already freed when freeContext() is called in QnnApi destructor
  // Skip deregistration to avoid "Failed to find memHandle" errors during cleanup
  // for (const auto& it : m_memHandleToRpcMem) {
  //   if (QNN_SUCCESS != m_qnnInterface->memDeRegister(&it.first, 1)) {
  //     QNN_ERROR("Failed to deregister ion memory with the backend");
  //   }
  // }
