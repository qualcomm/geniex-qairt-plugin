...
inline uint64_t align(uint64_t size, uint32_t dataAlignmentSize) {
  return dataAlignmentSize ? (size + dataAlignmentSize - 1) & ~(static_cast<uint64_t>(dataAlignmentSize) - 1) : size;
}
...
class RpcMem final : public IBufferAlloc {
 public:
  RpcMem(Qnn_ContextHandle_t contextHandle, QNN_INTERFACE_VER_TYPE* qnnInterface, uint32_t dataAlignmentSize = 0);
