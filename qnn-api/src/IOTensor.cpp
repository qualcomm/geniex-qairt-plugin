...
bool IOTensor::initialize(Qnn_ContextHandle_t contextHandle, uint32_t dataAlignmentSize) {
  if (m_bufferAlloc == BufferAlloc::SHARED_BUFFER) {
    m_bufferManager =
        std::unique_ptr<IBufferAlloc>(new RpcMem(contextHandle, m_qnnInterface, dataAlignmentSize));
  ...
  *tensors = (Qnn_Tensor_t*)calloc(1, tensorCount * sizeof(Qnn_Tensor_t));
  ...
  if (m_bufferAlloc == BufferAlloc::SHARED_BUFFER) {
    ...
    if (!skipBufferAllocation) {
      ...
      if (memPointer) {
        QNN_DEBUG("Successfully allocated a buffer of size %lu, pointer %p, fd %d",
...
bool IOTensor::mapFusedBufferOffset(
  ...
  for (const bool mode : {true, false}) {
    Qnn_Tensor_t* tensor_bank = (mode) ? graph_info->inputTensors : graph_info->outputTensors;
    uint32_t num_tensors      = (mode) ? graph_info->numInputTensors : graph_info->numOutputTensors;

    for (size_t tidx = 0; tidx < num_tensors; tidx++) {
      Qnn_Tensor_t& tensor_wrapper = tensor_bank[tidx];
      Qnn_Tensor_t* tensor         = &tensor_wrapper;
      std::string tensor_name      = std::string(QNN_TENSOR_GET_NAME(tensor_wrapper));
